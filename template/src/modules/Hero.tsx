			...
			<div className="relative z-10 text-center max-w-3xl mx-auto px-4">
				...
				{(date || time || location || address) && (
					<div className="flex gap-4 justify-center items-stretch flex-wrap">
						{date && (
							<div
								className="flex items-center gap-2 bg-zinc-400/50 backdrop-blur-md rounded-lg px-4 py-2"
								{...props}
							>
								<span className="text-2xl">📅</span>
								<span className="font-bold text-2xl text-zinc-50">{date}</span>
							</div>
						)}
						{time && (
							<div
							...
							>
								<span className="text-2xl">⏰</span>
								<span className="font-bold text-2xl text-zinc-50">{time}</span>
							</div>
						)}
						{(location || address) && (
							<div
								className="flex items-center gap-2 bg-zinc-400/50 backdrop-blur-md rounded-lg px-4 py-2  text-center md:text-left"
								{...props}
							>
