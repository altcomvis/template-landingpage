...
export function Schedule({ data, participants, ...props }: ScheduleProps) {
	...
	const timeWrapClass =
		contentAlign === "center"
			? "font-light text-xl flex gap-3 items-center justify-center z-10"
			: contentAlign === "right"
				? "w-20 md:w-32 mr-[5px] md:-mr-[3px] text-left font-light text-xl flex gap-3 items-center justify-start z-10"
				: "w-20 md:w-32 ml-[5px] md:-ml-[3px] text-right font-light text-xl flex gap-3 items-center justify-end";
		...
		<section className="py-16" id="schedule" {...props}>
			<div className="container mx-auto px-4 max-w-6xl">
				...
				<div className="relative">
					...
					<div
						className={`absolute top-0 bottom-0 w-px bg-(--text) ${timelineLineClass}`}
					/>
								...
								<div className={contentWrapClass}>
									{item.type === "panel" ? (
										<div className="bg-(--surface) border shadow-md rounded-xl p-6 relative overflow-hidden">
											...
											{/* 👥 Participantes */}
											<div
												className={`flex flex-wrap items-start gap-6 relative z-10 ${contentParticipantsClass}`}
											>
